...
int check_tlb(unsigned char logical_page){
...
int check_page_table(unsigned char page_number){
...
int main(int argc, const char *argv[])
{
    ...
    {
        int logical_address = atoi(buffer);//supposed to be unsigned char?
        total_addresses++;
        //logical page
        logical_page = logical_address / PAGE_SIZE;
        offset=logical_address%PAGE_SIZE;
        if(check_tlb(logical_page)!=-1){
            physical_page=check_tlb(logical_page);
            tlb_hits++;
        }
        else{
            if (!check_page_table(logical_page))
            {
                page_faults++;
