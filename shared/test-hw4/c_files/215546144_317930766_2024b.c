...
int main(int argc, const char *argv[])
{
    ...
    while (fgets(buffer, BUFFER_SIZE, input_fp) != NULL) 
    {
        ...
        // check if logical page in tlb
        int logical_page_found = 0;
        for (int i = 0; i < TLB_SIZE; i++) {
            if (tlb[i].logical == logical_page) {
                // found in tlb
                logical_page_found = 1;
                tlb_hits++;
        ...
        // if logical page wasn't found in tlb
        if (!logical_page_found) {
            if (pagetable[logical_page] == -1) {
                ...
                // copy backing page memory to main memory
        ...
        const int physical_address = physical_page * PAGE_SIZE + logical_address % PAGE_SIZE;
        const int value = main_memory[physical_address];
