...
int main(int argc, const char *argv[])
{
    ...
    while (fgets(buffer, BUFFER_SIZE, input_fp) != NULL)
    {
        ...
        if(flag==0)//logical page isnt in tlb
        {
            if(pagetable[logical_page]==-1)//page_fault exception
            {
                //à
                page_faults++;
            ...
            tlbindex++;
            int tlbentry=tlbindex%TLB_SIZE;
            tlb[tlbentry].logical=logical_page;
            tlb[tlbentry].physical=physical_page;
        }

        //5

        int physical_address=(physical_page*PAGE_SIZE)+offset;
