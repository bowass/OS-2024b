...
do
	...
	mkdir todo
	cp ./tests/* ./todo
	cp $filename ./todo/virtmem.c 2>/dev/null
	
	grade_one=0
	note=""
	
	gcc_output=$(gcc -Werror -lm -std=c99 ./todo/virtmem.c -o ./todo/test 2>&1)
	...
	if [[ "$comp_one" -ne 0 ]]; then
		...
		./todo/test ./todo/BACKING_STORE.bin ./todo/test$i.in > ./todo/test_temp$i.out &
		PID=$!
