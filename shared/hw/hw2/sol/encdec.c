...
int encdec_open(struct inode *inode, struct file *filp)
{
	...
	filp->private_data = (encdec_private_data*)kmalloc(sizeof(encdec_private_data), GFP_KERNEL); // HERE
	encdec_private_data* x = (encdec_private_data*)filp->private_data;
	if(minor==0)
    {
...
ssize_t encdec_read_caesar( struct file *filp, char *buf, size_t count, loff_t *f_pos )
{
	...
	if ((*f_pos) + count > memory_size)
		...
		new_count = count;
	y=copy_to_user(buf,buffer_caesar, new_count);
	//incode
	if (x->read_state == ENCDEC_READ_STATE_DECRYPT) {
		for (i = 0;i < count;i++) {
...
ssize_t encdec_read_xor(struct file* filp, char* buf, size_t count, loff_t* f_pos)
{
	int i, new_count, y;
	encdec_private_data* x = ((encdec_private_data*)filp->private_data);
	if (x->read_state == ENCDEC_READ_STATE_RAW) {
		for (i = 0;i < count;i++) {
			buf[i] = buf[i] ^ x->key;
		}
	}
	...
	if (*f_pos == memory_size)
		return -EINVAL;
	if ((*f_op) + count > memory_size)
		new_count = memory_size - (*f_pos);
	else
		new_count = count;
	y = copy_to_user(buf, buffer_caesar, count);
	if (y != 0)
		return -EFAULT;
	*f_pos += new_count;
}
